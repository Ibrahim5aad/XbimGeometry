...
namespace Xbim.Geometry.Engine.Interop.Factories
{
    ...
    /// <see cref="NativeShapeBinarySerializer"/> and boolean operations to
    /// <see cref="NativeBooleanFactory"/>.
    /// </summary>
    internal class NativeShapeFactory2 : IXShapeFactory
    {
        private readonly NativeModelGeometryService _modelService;
        private readonly ILogger _logger;

        public NativeShapeFactory2(NativeModelGeometryService modelService, ILogger logger)
        {
        ...
        {
            return NativeShapeBinarySerializer.FromBrep(shape);
        }

        public string Convert(IXShape shape)
        {
            if (shape is NativeShape ns)
                return NativeShapeBinarySerializer.ToBrep(ns);

            throw new ArgumentException("Shape must be a NativeShape instance.", nameof(shape));
        }
        ...
        {
            if (shape is not NativeShape ns)
                throw new ArgumentException("Shape must be a NativeShape instance.", nameof(shape));
            ...
            try
            {
                ...
                return NativeShapeFactory.WrapShape(movedHandle);
            }
            finally
            {
                locHandle.Dispose();
        ...
        {
            var geometryFactory = (NativeGeometryFactory)_modelService.GeometryFactory;
        ...
        {
            if (body is not NativeShape nsBody)
                throw new ArgumentException("Body must be a NativeShape instance.", nameof(body));
            if (tool is not NativeShape nsTool)
                throw new ArgumentException("Tool must be a NativeShape instance.", nameof(tool));
