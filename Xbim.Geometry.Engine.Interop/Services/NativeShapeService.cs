...
namespace Xbim.Geometry.Engine.Interop.Services
{
    ...
    /// </summary>
    internal class NativeShapeService : IXShapeService, IDisposable
    {
        private readonly ILoggerFactory _loggerFactory;
        private readonly ILogger _logger;
        private NativeContextHandle? _ctx;

        public NativeShapeService(ILoggerFactory loggerFactory)
        {
            _loggerFactory = loggerFactory ?? throw new ArgumentNullException(nameof(loggerFactory));
            _logger = loggerFactory.CreateLogger<NativeShapeService>();
        }
        ...
        public IXShape Convert(string brepString)
        {
            ...
            return NativeShapeFactory.WrapShape(handle);
        }
        ...
        public string Convert(IXShape shape)
        {
            ...
            var native = shape as NativeShape
                ?? throw new ArgumentException("Shape must be a NativeShape.", nameof(shape));
        ...
        public IXShape SetPlacement(IXShape shape, IIfcObjectPlacement placement)
        {
            ...
            var geoFactory = new Factories.NativeGeometryFactory(
        ...
        public bool IsFacingAwayFrom(IXFace face, IXDirection direction)
        {
            ...
            var native = face as NativeShape;
        ...
        public IXShape Combine(IEnumerable<IXShape> shapes)
        {
            ...
            for (int i = 0; i < shapeList.Count; i++)
            {
                if (shapeList[i] is NativeShape ns)
                    ...
                    throw new ArgumentException("All shapes must be NativeShape instances.");
            }
            ...
            return NativeShapeFactory.WrapShape(compoundHandle);
        }
        ...
        private IXShape BooleanOp(IXShape body, IXShape tool, double precision, BooleanOpDelegate op)
        {
            ...
            var nativeBody = body as NativeShape
                ?? throw new ArgumentException("Body must be a NativeShape.");
            var nativeTool = tool as NativeShape
                ?? throw new ArgumentException("Tool must be a NativeShape.");
